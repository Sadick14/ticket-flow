...
import { Calendar, Share2, Twitter, Facebook, Linkedin, LinkIcon, Globe, Loader2 } from 'lucide-react';
...
export default function NewsDetailsPage() {
    ...
    useEffect(() => {
        if (!loading) {
            const foundArticle = news.find(a => a.id === articleId);
            setArticle(foundArticle || null);
            setPageLoading(false);
        } else {
            // Keep loading if context is still loading
            setPageLoading(true);
        }
    }, [articleId, news, loading]);
