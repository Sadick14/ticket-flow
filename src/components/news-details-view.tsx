...
import { Calendar, Share2, Twitter, Facebook, Linkedin, LinkIcon, Globe } from 'lucide-react';
import { format } from 'date-fns';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from '@/components/ui/carousel';
import type { NewsArticle } from '@/lib/types';

export function NewsDetailsView({ initialArticle }: { initialArticle: NewsArticle }) {
  const { toast } = useToast();
  const [article] = useState<NewsArticle>(initialArticle);
