...
export default function DashboardEventsPage() {
  ...
  const { events, organizations, getTicketsByEvent, deleteEvent, loading: appLoading } = useAppContext();
  ...
  const organizationEvents = useMemo(() => 
    events.filter(event => event.organizationId === organizationId && event.status === 'active'),
  ...
  if (authLoading || appLoading || !organization) {
    return (
  ...
  const handleDelete = async (eventId: string) => {
    try {
      ...
      toast({ title: "Event Archived", description: "The event has been successfully archived." });
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: "Failed to archive the event." });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-gradient-to-br from-white to-slate-50 border-slate-200 shadow-lg">
        <CardHeader>
          <CardTitle className="text-slate-900 text-xl font-bold">Your Events for {organization.name}</CardTitle>
          <CardDescription className="text-slate-600">
            You are managing {organizationEvents.length} event(s).
          </CardDescription>
        </CardHeader>
        <CardContent>
          {organizationEvents.length > 0 ? (
                    ...
                    return (
                      <TableRow key={event.id} className="hover:bg-slate-50/50 transition-colors border-b border-slate-100">
                        ...
                        <TableCell className="text-right">
                          ...
                          <Button variant="ghost" size="icon" asChild className="hover:bg-slate-100 text-slate-600 hover:text-slate-900">
                            <Link href={`/dashboard/${organizationId}/edit/${event.id}`}><Edit className="h-4 w-4" /></Link>
                          </Button>
                          {isOwner && (
                            <AlertDialog>
                              ...
                              <AlertDialogContent className="bg-white border-slate-200">
                                <AlertDialogHeader>
                                  <AlertDialogTitle className="text-slate-900">Archive this event?</AlertDialogTitle>
                                  <AlertDialogDescription className="text-slate-600">
                                    This will hide the event from public view but will not delete its data. You can find it in the admin archive.
            ...
            <div className="text-center py-16 border-2 border-dashed border-slate-300 rounded-lg bg-gradient-to-br from-slate-50 to-white">
              ...
              <p className="mt-1 text-sm text-slate-600">This organization has not created any events yet.</p>
              <div className="mt-6">
                <Button asChild className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white">
                  <Link href={`/dashboard/${organizationId}/create`}>Create Your First Event</Link>
                </Button>
              </div>
            </div>
          )}
